...
class ZoteroLinkConverter:
    ...
    def __init__(self, verbose: bool = False):
        ...
        url_citekey_map = defaultdict(list)
        zotero_records = []
        ...
        for item in parent_items:
            ...
            # Map URLs to citekeys
            if url := item_data.get('url'):
                if norm_url := rfw.normalize_url(url):
                    url_citekey_map[norm_url].append(citekey)
                    
            zotero_records.append(record)

        # Create DataFrame and validate URLs
        self.zotero_items = pd.DataFrame(zotero_records)
        if url_conflicts := {u: c for u, c in url_citekey_map.items() if len(c) > 1}:
        ...
        # Filter valid items
        valid_citekeys = [c[0] for c in url_citekey_map.values()]
        has_url = self.zotero_items['citekey'].isin(valid_citekeys)
        self.zotero_items = self.zotero_items[has_url].reset_index(drop=True)
        
        # Initialize regex and caches
        self._note_url_zotero_cache = {}
        self._note_title_zotero_cache = {}
        self.body_link_re = re.compile(r'\[(.*?)\]\((https?://\S+)\)')
        self.source_link_re = re.compile(r'\[\((\d+)\)\s*(.*?)\]\((https?://\S+)\)')

        if verbose:
            print(f"Initialized with {len(self.zotero_items)} Zotero items")
...
def relink_perplexity_export_smc(input_file: str, output_file: str):
    ...
    for section in sections[1:]:  # Process each user section
        ...
        user_header = rfw.summarize_prompt(body, 10, "## AI answer", 2)
        
        processed_body, processed_sources, unsourced_links = converter.convert_content_links(body, sources)
        processed_body = rfw.setext_headers_to_atx(processed_body, 3)  # Convert setext to ATX headers

        if unsourced_links:
            log_missing_links.append(
                f"Body links not in source: " +
