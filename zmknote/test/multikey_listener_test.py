...
def show_overwrite_popup(citation_key, is_last_item, total_items, request_id):
...
def process_items(items, request_id):
    ...
    for index, item in enumerate(items):
        ...
        zotero_key = item.get('zoteroItemKey')
        citation_key = item.get('citationKey')
        
        if not zotero_key or not citation_key:
            logger.warning(f"[{request_id}] Missing required keys in item: {item}")
            continue
        
        logger.info(f"[{request_id}] Processing item {index+1}/{total_items}: {citation_key}")
        
        # Create filepath using pathlib
        filename = f"{citation_key}.json"
        ...
        try:
            ...
            with open(filepath, 'x') as f:
                ...
                logger.info(f"[{request_id}] Processed item: {citation_key} (Zotero key: {zotero_key})")
            ...
            # Show the popup
            action = show_overwrite_popup(citation_key, is_last_item, total_items, request_id)
